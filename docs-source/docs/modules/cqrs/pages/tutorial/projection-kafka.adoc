...
The new Projection will be similar to what we developed in the xref:tutorial/projection-query.adoc[previous step], but it will send the events to a Kafka topic instead of updating a database.

Add a `PublishEventsProjectionHandler` class that is the Projection `Handler` for processing the events:

[source,scala]
----
include::example$shopping-cart-service-scala/src/main/scala/sample/shoppingcart/PublishEventsProjectionHandler.scala[]
----
...
The tagging of the events is already in place from the xref:tutorial/projection-query.adoc#_tagging[previous step].

Place the initialization code of the Projection in an `PublishEventsProjection` object:

[source,scala]
----
include::example$shopping-cart-service-scala/src/main/scala/sample/shoppingcart/PublishEventsProjection.scala[]
----
...
Add the following to `src/main/resources/kafka.conf`, which is included from `application.conf`:
...
And the following to `src/main/resources/discovery.conf`, which is included from `application.conf`:

[source,conf]
----
include::example$shopping-cart-service-scala/src/main/resources/discovery.conf[tag=kafka]
----

Then you need to call the `PublishEventsProjection.init` from the `Guardian` in `Main.scala`:

[source,scala]
----
include::example$shopping-cart-service-scala/src/main/scala/sample/shoppingcart/Main.scala[tag=PublishEventsProjection]
----
...
Create a project named `shopping-analytics-service` in a separate directory in the same way as the `shopping-cart-service` was created. See xref:tutorial/dev-env.adoc[Build].
...
[source,scala]
----
include::example$shopping-analytics-service-scala/src/main/scala/sample/shoppinganalytics/ShoppingCartEventConsumer.scala[]
----
...
To run the new service we also need a `Main` class, which calls the initialization of the `ShoppingCartEventConsumer`.

[source,scala]
----
include::example$shopping-analytics-service-scala/src/main/scala/sample/shoppinganalytics/Main.scala[]
----
...
----
sbt "runMain sample.shoppinganalytics.Main 3551"
----

Try it with `grpcurl`. Add 1 pencil to a cart:

----
grpcurl -d '{"cartId":"cart1", "itemId":"pencil", "quantity":1}' -plaintext 127.0.0.1:8051 shoppingcart.ShoppingCartService.AddItem
----
...
TODO deploy the updated shopping-cart-service and the new shopping-analytics-service to the cloud and try it
