...
After adding the Cinnamon Agent as described above, make sure that you add the {cinnamon-docs}/instrumentations/instrumentations.html[Cinnamon instrumentations {tab-icon}, window="tab"] that are necessary for your application. In this example, we add instrumentations for Akka HTTP and Akka gRPC:
