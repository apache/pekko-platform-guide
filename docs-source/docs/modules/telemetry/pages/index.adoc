...
Telemetry makes it possible to gather metrics, events, and trace information from Akka, Scala, Play, and Lagom based applications, and then store such data in {cinnamon-docs}/introduction/introduction.html#backend-plugins[a metric backend] such as Prometheus. It collects runtime-information about your application using an agent that {cinnamon-docs}/introduction/introduction.html#instrumentations[instruments your code] and can be configured to tailor the application needs.
