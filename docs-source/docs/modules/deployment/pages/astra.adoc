...
[source,shell script]
----
kubectl create secret generic shopping-cart-service-astra-secret --from-literal=username=scott --from-literal=password=tiger --from-file=secureConnectBundle=/path/to/secure-connect.zip
----

To enable the Astra integration you define the name of the secret in `cloudCassandraAstra` of the CR:
...
The Akka Operator will automatically mount the secure connect bundle in the Pod and xref:main-config.adoc[provide the configuration] for the connection based on the Secret when the application starts the `ActorSystem`.
