...
The Pekko Operator can provide configuration for JDBC databases for use as the event store or in projection.
This can be either:
...
It creates all tables needed for Pekko Persistence as well as the offset store table for Pekko Projection.
...
To use the JDBC integration in the Pekko Operator you place the connection credentials in a
