...
There are 3 security groups for the EKS cluster and you should select the one with description "EKS created security group ...". The one that has a name that doesn't contain a `/`. Make a note of this security group id for the EKS cluster.
...
[source,shell script]
----
kubectl run -i --tty msk-mgmt --image=adoptopenjdk:11-jre-hotspot --restart=Never --rm -- bash
----
...
From https://console.aws.amazon.com/msk/home:[Amazon MSK console {tab-icon}, window="tab"] you find the connection URL in the "View client information". Copy the list of ZooKeeper host/port pairs.
...
To enable the Kafka integration you define the name of the secret in `kafka` of the CR:
