...
[source,cql]
----
CREATE KEYSPACE IF NOT EXISTS pekko_testing
...
The Pekko Operator will automatically xref:config-secret.adoc#main-conf[provide the configuration] from the Secret when the application starts the `ActorSystem`.
