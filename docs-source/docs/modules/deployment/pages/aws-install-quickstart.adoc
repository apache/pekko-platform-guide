...
3. Install the Pekko Cloud Platform Helm chart
...
Before installation, you must subscribe to the {aws-marketplace}[Pekko Cloud Platform {tab-icon}, window="tab"] on AWS Marketplace by clicking the Subscribe button on the product page and accepting the terms and conditions.
...
The fastest way to get started with Pekko Cloud Platform on EKS is to run the Pekko Cloud Platform Deployment Pulumi playbook.
...
Download (clone) the https://github.com/lightbend/pekko-cloud-platform-deploy[Pekko Cloud Platform Deployment Pulumi Playbook {tab-icon}, window="tab"] with https://git-scm.com/book/en/v2/Getting-Started-Installing-Git[`git` {tab-icon}, window="tab"].

[source,shell script]
----
git clone https://github.com/lightbend/pekko-cloud-platform-deploy
----
...
[source,shell script]
----
cd pekko-cloud-platform-deploy
----
...
In almost all cases resources will start with a prefix `acp-<STACK NAME>`, where `acp` stands for "Pekko Cloud Platform" and `<STACK NAME>` is the name you chose for the Pulumi stack.
...
View Live: https://app.pulumi.com/<USER NAME>/pekko-cloud-platform-deploy/<STACK NAME>/previews/f8c74175-2815-462a-9925-78022ad1c49d

     Type                                    Name                                                                                        Plan
 +   pulumi:pulumi:Stack                     pekko-cloud-platform-deploy-<STACK NAME>                                                     create
...
View Live: https://app.pulumi.com/<USER NAME>/pekko-cloud-platform-deploy/<STACK NAME>/updates/18

     Type                                    Name                                                                                        Status
 +   pulumi:pulumi:Stack                     pekko-cloud-platform-deploy-<STACK NAME>                                                     created
...
For example, to return the Kubernetes namespace that the Pekko Cloud Platform Operator is installed in:
...
[NOTE]
====
Kafka Kubernetes secrets are created in the Pekko Cloud Platform Operator namespace.
...
[NOTE]
====
JDBC (RDS) Kubernetes secrets are created in the Pekko Cloud Platform Operator namespace.
These must be copied to each microservice namespace that references them.
Here is one way to copy a resource from one namespace to another:
...
* `operatorNamespace` - Kubernetes namespace where the Pekko Cloud Platform operator is running
...
| `pekko.operator.namespace`
| The Kubernetes namespace to install the Pekko Cloud Platform operator into.
| No
| `"lightbend"`

| `pekko.operator.version`
| The version of the Pekko Cloud Platform operator to install.
| No
| `"1.1.22"`

| `pekko.operator.installTelemetryBackends`
