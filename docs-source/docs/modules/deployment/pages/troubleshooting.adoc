...
The `pekkomicroservices` CR has a status with the purpose to give an overview of the deployment status of the application.

[source,shell script]
----
kubectl get pekkomicroservices
...
[source,shell script]
----
kubectl get pekkomicroservices shopping-cart-service
NAME                    PHASE     REPLICAS   DESIRED   AGE     UP
...
[source,shell script]
----
kubectl get pekkomicroservices -o wide
...
See https://doc.pekko.io/docs/pekko/current/typed/cluster-membership.html#cluster-membership-service[`Akka Cluster Membership`] for more information.

Even more information with `kubectl describe`:

[source,shell script]
----
kubectl describe pekkomicroservices shopping-cart-service
----
...
[source,shell script]
----
kubectl get pekkomicroservices shopping-cart-service -o wide
NAME                    PHASE              REPLICAS   DESIRED   AGE   UP    NON-UP   UNREACHABLE  MESSAGE
shopping-cart-service   DeploymentFailed   2          2         7h25m 2     0        0            pekko.operator.action.ActionException: Action [GetAction] failed: jdbc Secret [shopping-cart-service-jdbc-zecret] doesn't exist
----
...
Events from the operator can be identified as from the `pekko-platform-operator` source.


[source,shell script]
----
kubectl describe pod -l app.kubernetes.io/name=pekko-operator
Name:         pekko-operator-74fb867bb4-bhtrp
Namespace:    pekko-dev
Priority:     0
Node:         minikube/172.17.0.3
Start Time:   Thu, 03 Dec 2020 14:53:41 -0500
Labels:       app.kubernetes.io/instance=pekko-operator
              app.kubernetes.io/name=pekko-operator
...
IPs:
  IP:           172.18.0.5
Controlled By:  ReplicaSet/pekko-operator-74fb867bb4
Containers:
  pekko-operator:
    Container ID:   docker://93eb0288363af22ae95e6fe33777693f5cdb49659e6977e40cdd5e427d16af4c
    Image:          lightbend/pekko-operator:latest
    Image ID:       docker://sha256:330484db906194a55a7bca036911e029d1f7188a3e195dc3bfd63bbb63776572
...
Events:
  Type    Reason                    Age                From                    Message
  ----    ------                    ----               ----                    -------
  Normal  Created                   28s                kubelet, minikube       Created container pekko-operator
  Normal  Scheduled                 28s                default-scheduler       Successfully assigned pekko-dev/pekko-operator-74fb867bb4-bhtrp to minikube
  Normal  Pulled                    28s                kubelet, minikube       Container image "lightbend/pekko-operator:latest" already present on machine
  Normal  Started                   27s                kubelet, minikube       Started container pekko-operator
  Error   ReconciliationFailure     16s (x3 over 17s)  pekko-platform-operator  Reconciliation for [shopping-cart-service] failed with [java.lang.RuntimeException] we have a problem
  Normal  ReconciliationSuccessful  14s (x2 over 16s)  pekko-platform-operator  Reconciliation for [shopping-cart-service] was successful
  Normal  ReconciliationSuccessful  13s (x3 over 17s)  pekko-platform-operator  Reconciliation for [shopping-order-service] was successful
  Error   ReconciliationFailure     13s (x2 over 14s)  pekko-platform-operator  Reconciliation for [shopping-order-service] failed with [java.lang.RuntimeException] we have a problem
----
...
[source,shell script]
----
$ kubectl get events --watch --selector app.kubernetes.io/name=pekko-platform-operator -n lightbend
LAST SEEN   TYPE     REASON                     OBJECT                               MESSAGE
22m         Error    ReconciliationFailure      pod/pekko-operator-74fb867bb4-bhtrp   Reconciliation for [shopping-cart-service] failed with [java.lang.RuntimeException] we have a problem
13m         Error    ReconciliationFailure      pod/pekko-operator-74fb867bb4-bhtrp   Reconciliation for [shopping-order-service] failed with [java.lang.RuntimeException] we have a problem
13m         Normal   ReconciliationSuccessful   pod/pekko-operator-74fb867bb4-bhtrp   Reconciliation for [shopping-order-service] was successful
13m         Normal   ReconciliationSuccessful   pod/pekko-operator-74fb867bb4-bhtrp   Reconciliation for [shopping-cart-service] was successful
----
