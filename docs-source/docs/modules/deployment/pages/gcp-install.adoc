...
From a terminal, login to your project using the gcloud CLI, follow the quick start at https://cloud.google.com/sdk/docs/quickstart[https://cloud.google.com/sdk/docs/quickstart {tab-icon}, window="tab"] if you are not familiar with it yet.
...
[source,shell script]
----
gcloud container clusters get-credentials my-cluster --zone us-west1-c --project my-project
----
Execute the command from a terminal to connect to the Kubernetes cluster. You are now ready to start using the Akka Cloud Platform. 
