...
[source,shell script]
----
kubectl apply -f  https://raw.githubusercontent.com/lightbend/akka-operator-helm/v1.1.19/akka-operator/crds/v1/akka-microservices-crd.yml # <1>
----
<1> `v1.1.19` is the latest available version published to GCP (see steps above)
...
----
<1> `1.1.19` is the latest available version published to GCP (see steps above)
...
kubectl apply -f  https://raw.githubusercontent.com/lightbend/akka-operator-helm/v1.1.19/akka-operator/crds/v1/akka-microservices-crd.yml # <1>
----
<1> `v1.1.19` is the version published to GCP (see steps above)
...
helm upgrade akka-operator akka-operator-helm/akka-operator \
  ...
  --set reportingSecret=akka-cloud-platform-1-license  # <2>
----
<1> `1.1.19` is the version published to GCP (see steps above)
<2> `akka-cloud-platform-1-license` is the previously created secret name (see steps above)
