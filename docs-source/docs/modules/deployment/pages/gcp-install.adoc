...
. An App instance name — Select a unique name. We recommend using the default or `pekko-platform-operator-1`.
. The pekko-operator.serviceAccount.name — This is the name of the service account used by the operator to access Kubernetes resources.
...
. The output will be similar to the following snippet. Take note of the secret name for future reference, in the case below the name is: `pekko-cloud-platform-1-license`.
+
[source,shell script]
----
secret/pekko-cloud-platform-1-license created
----
...
. Verify the latest version already published to GCP https://console.cloud.google.com/gcr/images/cloud-marketplace/GLOBAL/lightbend-public/pekko-cloud-platform?gcrImageListsize=30[here] and take note of the it.

. Update the Akka Microservices CRD to latest:
+
[source,shell script,subs="attributes+"]
----
kubectl apply -f  https://raw.githubusercontent.com/lightbend/pekko-operator-helm/v{pekko-cloud-platform-version}/pekko-operator/crds/v1/pekko-microservices-crd.yml # <1>
----
<1> `v{pekko-cloud-platform-version}` is the latest available version published to GCP (see steps above)
...
helm install pekko-operator pekko-operator-helm/pekko-operator \
  --namespace lightbend \
  --version={pekko-cloud-platform-version} \  # <1>
  --set provider.name=gcp \
  --set reportingSecret=pekko-cloud-platform-1-license  # <2>
----
<1> `{pekko-cloud-platform-version}` is the latest available version published to GCP (see steps above)
<2> `pekko-cloud-platform-1-license` is the previously created secret name (see steps above)
...
. Verify the version you want to deploy is already published to GCP https://console.cloud.google.com/gcr/images/cloud-marketplace/GLOBAL/lightbend-public/pekko-cloud-platform?gcrImageListsize=30[here] and take note of the it.
...
kubectl apply -f  https://raw.githubusercontent.com/lightbend/pekko-operator-helm/v{pekko-cloud-platform-version}/pekko-operator/crds/v1/pekko-microservices-crd.yml # <1>
----
<1> `v{pekko-cloud-platform-version}` is the version published to GCP (see steps above)
...
helm upgrade pekko-operator pekko-operator-helm/pekko-operator \
  ...
  --set reportingSecret=pekko-cloud-platform-1-license  # <2>
----
<1> `{pekko-cloud-platform-version}` is the version published to GCP (see steps above)
<2> `pekko-cloud-platform-1-license` is the previously created secret name (see steps above)
