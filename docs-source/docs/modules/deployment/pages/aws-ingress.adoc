...
Please make sure you fill in a value for every field requested when creating the certificate and private key, and use a domain name formatted value for the 'Common Name', for instance, "example.com".  

You can import the cert into ACM with the following command:

[source,shell script]
----
aws acm import-certificate --certificate fileb://MyCertificate.crt --private-key fileb://MyKey.key --region
----
