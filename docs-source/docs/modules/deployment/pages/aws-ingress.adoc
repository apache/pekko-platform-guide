...
To expose gRPC or HTTPS to the internet you can use the https://kubernetes-sigs.github.io/aws-load-balancer-controller/latest/[AWS Load Balancer Controller {tab-icon}, window="tab"] with a https://kubernetes.io/docs/concepts/services-networking/ingress/[Kubernetes Ingress {tab-icon}, window="tab"] created by the Akka Platform Operator.
...
When the deployment descriptor has been applied the Akka Platform Operator will create an Ingress and you can retrieve the public address with:
