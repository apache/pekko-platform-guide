...
. Authenticate docker to the Amazon ECR repository for Akka Cloud Platform:
+
[source,shell script]
----
aws ecr get-login-password --region eu-central-1 | docker login --username AWS --password-stdin 709825985650.dkr.ecr.us-east-1.amazonaws.com
----
