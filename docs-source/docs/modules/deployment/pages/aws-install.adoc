...
After the Akka Cloud Platform Helm chart is installed you observe an `ImagePullBackOff` status on a pod for the `akka-operator` `Deployment`. When you describe the pod you find a `Failed` event with a docker pull error message that mentions _"You are not entitled to pull this AWS Marketplace product image"_. This will occur when your AWS account is not subscribed to the Akka Cloud Platform on AWS Marketplace. To resolve you must xref:deployment:aws-install.adoc#_amazon_marketplace[subscribe to the product] and then wait for the `akka-operator` pod to restart and attempt to pull the image again.
