...
Exporting metrics to Prometheus requires opening a port on your Pods for Prometheus to scrape the metrics data gathered.
...
Specifying the Prometheus port the Akka Operator will open the port on your application's Pods and annotate the pods appropriately for Prometheus to locate them in the Kubernetes cluster and scrape them.
