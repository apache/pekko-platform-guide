...
You can view the https://lightbend.github.io/pekko-operator-helm/pekko-operator/crds/v1/pekko-microservices-crd.yml[schema definition of the PekkoMicroservices deployment descriptor (CRD) {tab-icon}, window="tab"]. A https://lightbend.github.io/pekko-operator-helm/api-reference/v1/shopping-cart-service-full-cr.yml[deployment descriptor (CR) containing all properties {tab-icon}, window="tab"] provides a factitious example.
...
[source,shell script]
----
kubectl scale --replicas=3 pekkomicroservices shopping-cart-servics
----

=== Scale Akka Cluster roles

{pekko}/typed/cluster.html#node-roles[Akka Cluster node roles {tab-icon}, window="tab"] can be used to scale portions of your application independently. For example, scaling persistent entities separately from projections.
...
The Akka Operator will automatically provide the `pekko.cluster.roles` configuration to the application. Roles defined in `application.conf` will be overridden.
...
The Akka Operator adds a label `pekko.lightbend.com/cluster-role` to the Pods, which for example can be used to select Pods with a certain role:

[source,shell script]
----
kubectl get pods -label pekko.lightbend.com/cluster-role=entity
----
...
A custom Logback configuration can be provided to the deployment without building a new image. Make sure that your application is already configured to use the {pekko}/typed/logging.html#slf4j-backend[Slf4j backend {tab-icon}, window="tab"] for logging.
...
--set java_tool_options="\
   -Dpekko.operator.application-namespace=the-app-ns \
