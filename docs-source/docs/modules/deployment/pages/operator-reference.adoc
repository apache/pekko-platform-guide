...
Exporting metrics to Prometheus requires opening a port on your Pods for Prometheus to scrape the metrics data gathered. The Akka Operator assumes you will be using https://developer.lightbend.com/docs/telemetry/current/plugins/prometheus/prometheus.html#prometheus[Lightbend Telemetry's support for Prometheus]. You have to configure the build of your application following the Lightbend Telemetry docs. You should also make sure to enable the `http-server` exporter. Then, you need to open the prometheus port:
...
Specifying the Prometheus port the Akka Operator will open the port on your application's Pods and annotate the pods appropriately for Prometheus to locate them in the Kubernetes cluster and scrape them. The Akka operator will also create and inject an Environment Variable with the port value so you can setup your configuration to read from that value:
