...
NOTE: You may need to run `pulumi destroy` a couple of times to destroy all resources effectively. Usually, you can run with `--refresh` flag to refresh the state of the stack's resources before destroying resources. For example, `pulumi destroy --refresh`.
