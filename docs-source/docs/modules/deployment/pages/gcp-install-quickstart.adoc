...
* Download (clone) the https://github.com/lightbend/pekko-cloud-platform-deploy[Akka Cloud Platform Deployment Pulumi Playbook {tab-icon}, window="tab"] with https://git-scm.com/book/en/v2/Getting-Started-Installing-Git[`git` {tab-icon}, window="tab"].
...
----
pulumi config set pekko-cloud-platform-gcp-deploy:operator-version {pekko-cloud-platform-version}
----
...
View Live: https://app.pulumi.com/<USER NAME>/pekko-cloud-platform-gcp-deploy/<STACK NAME>/previews/f7a820f3-0fd8-4f2e-903a-fecdd3c72749

     Type                                                                  Name                                                                                       Plan
 +   pulumi:pulumi:Stack                                                   pekko-cloud-platform-gcp-deploy-<STACK NAME>                                                create
...
...
Do you want to perform this update? yes
     ...
     Type                                                                 Name                                                                                        Status
 +   pulumi:pulumi:Stack                                                  pekko-cloud-platform-gcp-deploy-<STACK NAME>                                                 created
...
is implemented as a sidecar. Hence you may see the following failure in `pulumi up` where the update of the stack fails for pekko-operator:

[source,shell script]
----
Diagnostics:
  kubernetes:apps/v1:Deployment (lightbend/pekko-operator):
    error: 4 errors occurred:
    	* resource lightbend/pekko-operator was successfully created, but the Kubernetes API server reported that it failed to fully initialize or become live: 'pekko-operator' timed out waiting to be Ready
    	* [MinimumReplicasUnavailable] Deployment does not have minimum availability.
    	* Minimum number of live Pods was not attained
    	* [Pod lightbend/pekko-operator-9b6f567cc-psqhh]: containers with unready status: [pekko-operator ubbagent]
 
  pulumi:pulumi:Stack (pekko-cloud-platform-gcp-deploy-dev):
    error: update failed
----
...
  	* updating urn:pulumi:dev::pekko-cloud-platform-gcp-deploy::gcp:sql/databaseInstance:DatabaseInstance::pg12instance: 1 error occurred:
