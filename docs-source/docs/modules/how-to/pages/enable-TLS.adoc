...
In the xref:shopping-microservices:index.adoc[Microservices Tutorial] there's a guide on creating a xref:shopping-microservices:grpc-server.adoc#_server[gRPC server] using {akka-http}[Akka HTTP] and {akka-grpc}[Akka gRPC]. That example, though, will only bind a server supporting plaintext HTTP.
