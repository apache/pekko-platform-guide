...
In the xref:shopping-microservices:index.adoc[Microservices Tutorial] there's a guide on creating a xref:shopping-microservices:grpc-server.adoc#_server[gRPC server] using link:{akka-http}[Akka HTTP] and link:{akka-grpc}[Akka gRPC]. That example, though, will only bind a server supporting plaintext HTTP.
...
Follow the link:{akka-http}/server-side/server-https-support.html[Akka HTTP documentation] for details on each of those steps.
...
NOTE: See also the documentation on link:{akka-http}/current/server-side/http2.html#use-newserverat-bind-and-https[using the `newServerAt(...).bind()` API when serving HTTPS].
...
If you want to link:{akka-http}/server-side/server-https-support.html#running-both-http-and-https[run both HTTP and HTTPS side by side] you must create two `Http` instances and bind each instance to a different port. Both instances should share a single `Route` instance.
