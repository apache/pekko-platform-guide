...
This guide describes how to use Cassandra instead of Relational Database. It assumes that you created the project using the xref:microservices-tutorial:index.adoc[Implementing Microservices with Pekko tutorial] and it describes the changes relative to the JDBC setup that is used in the tutorial.
...
Replace `pekko-persistence-jdbc` with `pekko-persistence-cassandra`:
...
[source,xml,indent=0]
----
include::example$shopping-cart-service-cassandra-java/pom.xml[tag=pekko-persistence-cassandra]
----
...
----
include::example$shopping-cart-service-cassandra-scala/build.sbt[tag=pekko-persistence-cassandra]
----

=== Configuration

Change the configuration in `src/main/resources/persistence.conf` to the following to enable `pekko-persistence-cassandra`:

.src/main/resources/persistence.conf
[source,hocon]
----
include::example$shopping-cart-service-cassandra-scala/src/main/resources/persistence.conf[tag=pekko-persistence-cassandra]
----
...
Replace `pekko-projection-jdbc` with `pekko-projection-cassandra`:
...
[source,xml,indent=0]
----
include::example$shopping-cart-service-cassandra-java/pom.xml[tag=pekko-projection-cassandra]
----
...
[source,scala,indent=0]
----
include::example$shopping-cart-service-cassandra-scala/build.sbt[tag=pekko-projection-cassandra]
----

=== Configuration

Change the configuration in `src/main/resources/persistence.conf` and add following for `pekko-projection-cassandra`:

.src/main/resources/persistence.conf
[source,hocon]
----
include::example$shopping-cart-service-cassandra-scala/src/main/resources/persistence.conf[tag=pekko-projection-cassandra]
----
...
----
+
<1> extends `pekko.projection.javadsl.Handler`
...
----
+
<1> extends `pekko.projection.scaladsl.Handler`
<2> the `process` method to implement
...
----
+
The `CassandraSession` comes from the Cassandra connector in Pekko-connectors and provides an asynchronous API for executing __CQL__ statements to Cassandra. In the initialization code, introduced later, we will see how to get access to a `CassandraSession`. You can learn more about the `CassandraSession` in the {alppekko}/cassandra.html[Pekko-connectors reference documentation {tab-icon}, window="tab"].
...
----
+
The `CassandraSession` comes from the Cassandra connector in Pekko-connectors and provides an asynchronous API for executing __CQL__ statements to Cassandra. In the initialization code, introduced later, we will see how to get access to a `CassandraSession`. You can learn more about the `CassandraSession` in the {alppekko}/cassandra.html[Alppekko reference documentation {tab-icon}, window="tab"].
...
----
+
<1> `ShardedDaemonProcess` will manage the Projection instances. It ensures the Projection instances are always running and distributes them over the nodes in the Pekko Cluster.
...
`create_tables.cql` will create the keyspace and all tables needed for pekko Persistence as well as the offset store table for Pekko Projection.
...
====
It will create the keyspace and all tables needed for pekko Persistence as well as the offset store table for Pekko Projection.
+
[CAUTION]
====
The keyspace as created by the script works fine for local development but is probably not what you need in a production environment.
...
Create a Cassandra cluster and credentials secret as described in xref:deployment:astra.adoc[DataStax Astra].

FIXME more instructions

== Learn more

* {pekko-persistence-cassandra}[Pekko Persistence Cassandra plugin reference documentation {tab-icon}, window="tab"]
* {pekko-projection}/[pekko Projection reference documentation {tab-icon}, window="tab"].
