...
[NOTE]
====
This howto is not about {reactive-microservices-ebook}[slicing the monolith into Microservices] {reactive-microservices-ebook}[{tab-icon}, window="tab"] or the xref:concepts:event-sourcing.adoc#advantages[Advantages of Event Sourcing].
====
