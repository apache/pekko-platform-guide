...
Not all nodes in an Pekko Cluster need to be identical. When starting each process you can add some metadata to it to give that instance a special purpose. {pekko}/typed/cluster.html#node-roles[Node Roles {tab-icon}, window="tab"] in Pekko Cluster will handle that metadata. The code (image) should still be the same for all roles, but it can select behavior at runtime based on the role configuration.
...
* serves gRPC traffic,
* hosts `Cart Entity` 's (that is, {pekko}/typed/persistence.html[`EventSourcedBehavior` 's {tab-icon}, window="tab"])
* runs three different {pekko-projection}[projections {tab-icon}, window="tab"].
...
You can {pekko}/typed/cluster-sharding.html[specify a role when declaring the entity {tab-icon}, window="tab"]:
...
When using the xref:deployment:index.adoc[Pekko Operator] you should define the roles in the deployment descriptor:
...
<1> total of 10 replicas means that 10 Pods, corresponding to 10 Pekko Cluster members, will be started
...
The Pekko Operator will automatically provide the `pekko.cluster.roles` configuration to the application.

If you are not using the Pekko Operator you would have to define the roles in configuration or from the command line:

[source]
----
... -Dpekko.cluster.roles.0=entity ...
----
...
When using the xref:deployment:index.adoc[Pekko Operator] you should define the roles in the deployment descriptor as described for the `entity` role above:
...
[source]
----
... -Dpekko.cluster.roles.0=projection ...
----


== Learn more

* {pekko}/typed/cluster.html#node-roles[Pekko Cluster Node Roles {tab-icon}, window="tab"]
* xref:deployment:operator-reference.adoc#scale[Scale the number of Pods]
* {pekko}/typed/cluster-sharding.html[Pekko Cluster Sharding {tab-icon}, window="tab"]
* {pekko}/typed/cluster-sharded-daemon-process.html#sharded-daemon-process[Sharded Daemon Process {tab-icon}, window="tab"]
