...
To apply customizations on top of a default deployment descriptor (CR) we recommend using https://kubectl.docs.kubernetes.io/guides/introduction/kustomize/[Kustomize], built into `kubectl`.

Prepare a folder called `base` containing the following files:

The default deployment descriptor (CR):

.base/shopping-cart-service.yml
[source,yaml]
----
