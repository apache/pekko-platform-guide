...
The project produced with the xref:microservices-tutorial:template.adoc#seed-template[Akka Microservicies template] assumes the application will use the {akka-persistence-cassandra}[Akka Persistence Cassandra plugin {tab-icon}, window="tab"]. You can opt out from using that plugin and use the {akka-persistence-jdbc}[Akka Persistence JDBC {tab-icon}, window="tab"] or other plugins instead.
...
The project produced with the xref:microservices-tutorial:template.adoc#seed-template[Akka Microservicies template] assumes the application will use a Cassandra database to store the {akka-projection}/overview.html#overview[offset tracking {tab-icon}, window="tab"] required by the projections. You can replace the artifact to use an RDBMS:
