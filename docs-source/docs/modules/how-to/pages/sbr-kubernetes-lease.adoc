...
<1> When using a kubernetes lease as a backend for the Split-Brain Resolver, the poerator will make sure the Lease exists and will name it `<metadata.name>-akka-sbr`. In this case, the lease will be `shopping-cart-service-akka-sbr`.