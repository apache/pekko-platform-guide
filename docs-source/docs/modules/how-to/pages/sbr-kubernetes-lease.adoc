...
When using a Kubernetes-backed Lease to improve the SBR decision process, you must make sure the name of your `ActorSystem` is the same for all the nodes of the Akka Cluster. At same time, you must make sure that different Akka Clusters deployed in the same Kubernetes cluster use different names. Each Akka Cluster must have a single Lease, and each Lease is identified by the name of the `ActorSystem`.
  ...
  splitBrainResolver:
    lease:
      provider: "kubernetes" # <1>
        actorSystemName: "ShoppingCartService" # <2>
----

<1> Specify the Lease provider is `"kubernetes"` to create ne lease instance and setup the necessary `Role` and `RoleBinding`for your pods.
<2> When using a lease as a backend for the Split-Brain Resolver, the lease name must be `<actorSystem.name>-akka-sbr`. In this case, the lease will be `ShoppingCartService-akka-sbr`.