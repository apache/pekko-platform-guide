...
An Event Sourced entity is implemented with the `EventSourcedBehavior` and defines command handlers:
...
To put this together we need some initialization code:
...
Then you need to call the `ShoppingCart.init` from the `Guardian` in `Main.scala`:
