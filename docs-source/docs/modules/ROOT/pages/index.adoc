...
Lightbend Telemetry can capture data for {akka-http}/introduction.html[Akka HTTP and Akka gRPC {tab-icon}, window="tab"] related features. For those interested in adding telemetry to their deployments we advise reviewing the xref:telemetry:index.adoc[Telemetry] section of this guide.
