...
NOTE: This concept is related to {reactive-manifesto}[resilience {tab-icon}, window="tab"] and {reactive-manifesto}[elasticity {tab-icon}, window="tab"] which are two properties of {reactive-manifesto}[Reactive Systems {tab-icon}, window="tab"].

Isolation is a prerequisite for {reactive-manifesto}[resilience {tab-icon}, window="tab"] and {reactive-manifesto}[elasticity {tab-icon}, window="tab"] and requires {reactive-manifesto}glossary#Asynchronous[asynchronous communication {tab-icon}, window="tab"] boundaries between services to decouple them in:
