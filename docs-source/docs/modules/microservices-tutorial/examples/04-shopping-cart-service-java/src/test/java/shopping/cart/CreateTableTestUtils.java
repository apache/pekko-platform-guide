...
public class CreateTableTestUtils {
    public static void createTables(ActorSystem<?> system) throws Exception {
        ...
        session.executeDDL("CREATE TABLE IF NOT EXISTS " + keyspace + "." + ItemPopularityRepositoryImpl.POPULARITY_TABLE + " (\n" +
                "item_id text,\n" +
                "count counter,\n" +
                "PRIMARY KEY (item_id))"
        ).toCompletableFuture().get(30, SECONDS);

        LoggerFactory.getLogger(CreateTableTestUtils.class).info("Created keyspace [{}] and tables", keyspace);
    }
