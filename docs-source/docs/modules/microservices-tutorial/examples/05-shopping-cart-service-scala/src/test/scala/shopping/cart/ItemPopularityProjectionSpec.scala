...
object ItemPopularityProjectionSpec {
  ...
  class TestItemPopularityRepository
    ...
    override def update(
      ...
      Future.successful {
        counts = counts + (itemId -> (counts.getOrElse(
          itemId,
          0L) + delta))
        Done
      }
      ...
      val events =
        Source(
          List[EventEnvelope[ShoppingCart.Event]](
            createEnvelope(
              ShoppingCart
                .ItemAdded("a7098", "bowling shoes", 1),
              0L),
            createEnvelope(
            ...
            createEnvelope(
              ShoppingCart
                .ItemAdded("0d12d", "akka t-shirt", 1),
              3L),
            createEnvelope(
              ShoppingCart.ItemAdded("0d12d", "skis", 1),
              4L),
            createEnvelope(
              ShoppingCart.ItemRemoved("0d12d", "skis", 1),
              5L),
            createEnvelope(
              ShoppingCart.CheckedOut(
                "0d12d",
