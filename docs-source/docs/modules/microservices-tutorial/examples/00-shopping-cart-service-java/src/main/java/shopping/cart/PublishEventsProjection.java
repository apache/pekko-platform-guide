...
public final class PublishEventsProjection {
    ...
    public static void init(ActorSystem<?> system) {
        ...
        ShardedDaemonProcess.get(system).init(
                ...
                index -> ProjectionBehavior.create(createProjectionFor(system, topic, sendProducer, index)),
    ...
    private static SendProducer<String, byte[]> createProducer(ActorSystem<?> system) {
        ...
        CoordinatedShutdown.get(system).addTask(CoordinatedShutdown.PhaseActorSystemTerminate(), "close-sendProducer", () -> sendProducer.close());
        return sendProducer;
    }


    private static AtLeastOnceProjection<Offset, EventEnvelope<ShoppingCart.Event>> createProjectionFor(ActorSystem<?> system, String topic, SendProducer<String, byte[]> sendProducer, int index) {
