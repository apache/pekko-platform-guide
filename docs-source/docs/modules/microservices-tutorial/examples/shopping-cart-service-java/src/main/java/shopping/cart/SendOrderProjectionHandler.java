...
public final class SendOrderProjectionHandler extends Handler<EventEnvelope<ShoppingCart.Event>> {
    ...
    @Override
    public CompletionStage<Done> process(EventEnvelope<ShoppingCart.Event> envelope) throws Exception {
    ...
    private CompletionStage<Done> sendOrder(ShoppingCart.CheckedOut checkout) {
        EntityRef<ShoppingCart.Command> entityRef = sharding.entityRefFor(ShoppingCart.ENTITY_KEY, checkout.cartId);
        CompletionStage<ShoppingCart.Summary> reply = entityRef.ask(replyTo -> new ShoppingCart.Get(replyTo), timeout);
        return reply.thenCompose(cart -> { // <2>
