...
=== Akka Workshop
The fourth video of the https://info.lightbend.com/pekko-platform-workshop-part-4-on-demand-recording.html[Akka Workshop Series {tab-icon}, window="tab"] covers CQRS and Projections for Kafka and gRPC. It provides some solid guidance to aid you in understanding this section, and the next section of this guide.
...
ifdef::todo[TODO: add a sentence or two about using the Alppekko Kafka connector, benefits, etc, maybe a link to the doc.]
...
<1> `SendProducer` comes from the Kafka connector in Alppekko.
...
. Create a `ShoppingCartEventConsumer` [.group-scala]#object# [.group-java]#class# in `shopping-analytics-service`. It runs an Akka Stream with a Kafka `Consumer.committableSource` from Alppekko Kafka.
+
[.tabset]
Java::
+
