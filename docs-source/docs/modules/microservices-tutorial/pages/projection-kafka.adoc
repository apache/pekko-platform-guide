...
ifdef::todo[TODO: add a sentence or two about using the Pekko Kafka connector, benefits, etc, maybe a link to the doc.]
...
<1> `SendProducer` comes from the Kafka connector in Pekko connectors.
...
. Create a `ShoppingCartEventConsumer` [.group-scala]#object# [.group-java]#class# in `shopping-analytics-service`. It runs an Pekko Stream with a Kafka `Consumer.committableSource` from Alppekko Kafka.
+
[.tabset]
Java::
+
