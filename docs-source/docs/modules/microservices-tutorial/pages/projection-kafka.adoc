...
Follow the xref:deployment:aws-msk.adoc[Amazon MSK] instructions to setup a Kafka cluster in EKS and a secret to access it.

The xref:deployment:aws-msk.adoc[Amazon MSK] page also covers how to create the `shopping-cart-events` topic.
...
Update the `shopping-cart-service/kubernetes/shopping-cart-service-cr.yml` and `shopping-analytics-service/kubernetes/shopping-analytics-service-cr.yml` deployment descriptors with the respective image tag and the Kafka credential secret, for example:
