...
As described in xref:concepts:internal-and-external-communication.adoc[Internal and External Communication concepts] we can publishing messages to a broker, such as Apache Kafka, to decouple communication between different Microservices.
...
When finished, stop the `shopping-cart-service` and `shopping-analytics-service` with `ctrl-c`. Leave Cassandra and Kafka running for the next set of steps, or stop it with:
