...
We will create the gRPC `ShoppingCartService` for the *Cart Service* shown in the architectural overview below. The motivation for using gRPC as the inter-service protocol is explained in xref:concepts:pekko-grpc.adoc[Pekko gRPC concepts]. The {pekko-blog}/news/2020/04/08/pekko-grpc-intro-video[Introduction to Pekko gRPC video {tab-icon}, window="tab"] is also a good starting point for learning Pekko gRPC.
...
The Pekko gRPC plugin will generate code from the service descriptor when you compile the project. Run [.group-scala]#`sbt`# [.group-java]#`mvn`# to compile:
...
We will run the gRPC service implementation in an Pekko HTTP server. Add the following server initialization code in a `ShoppingCartServer` [.group-scala]#`object`# [.group-java]#`class`#:
...
<2> Initialization of Pekko Management that is used for forming the Pekko Cluster.
...
We suggest that you read the xref:deployment:index.adoc[] section to get an understanding of how deployment with the Pekko Cloud Platform works.
...
Before following the steps below, create a Kubernetes cluster and install the Pekko Operator. Used the instructions below for:
...
In `kubernetes/shopping-cart-service-cr.yml` you will find the deployment descriptor of the `PekkoMicroservice` that the Pekko Operator will use when deploying the application in Kubernetes.
...
The Pekko Operator will notice the deployment descriptor and deploy the `shopping-cart-service`. You can see progress by viewing the status:
...
* xref:concepts:pekko-grpc.adoc[Pekko gRPC concepts]
* {pekko-grpc}/server/index.html[Pekko gRPC server reference documentation {tab-icon}, window="tab"].
