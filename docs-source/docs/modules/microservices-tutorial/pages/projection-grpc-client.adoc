...
. Add port forwarding for the gRPC endpoint for both services from a separate terminal:
+
[source,shell script]
----
...
[source,shell script]
----
kubectl port-forward svc/shopping-order-service-grpc 8301:8301
----

include::projection-grpc-client.adoc[tag=exercise]
