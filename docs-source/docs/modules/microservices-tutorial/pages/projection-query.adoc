...
Next, we will create an {pekko-projection}/[Pekko Projection {tab-icon}, window="tab"] from the events emitted by the `ShoppingCart` entity. The Projection will update counts in the database to track item popularity. Then, we can query the database to find how popular an item is. Since `ShoppingCart` entities can only be addressed by individual cart identifiers, we can find a particular cart, but we can't find all carts that contain a particular item.
...
* distribute the Projection instances over the nodes in the Pekko Cluster
* work with the Projection JDBC API

The xref:concepts:cqrs.adoc[CQRS] section explains why it is a good practice to build a Projection from entity events that can be queried. The {pekko-blog}/news/2020/09/10/pekko-projection-intro-video[Introduction to Pekko Projections video {tab-icon}, window="tab"] is also a good starting point for learning about Pekko Projections.
...
----
+
<1> Extends `org.apache.pekko.projection.javadsl.JdbcHandler`.
...
NOTE: In this example, we use five different tags. Tagging is not easy to change later without system downtime. Before going live in production you should consider how many tags to use, see {pekko-projection}/running.html[Pekko Projections reference documentation {tab-icon}, window="tab"] for more information.
...
[IMPORTANT]
====
Calls to the repository are blocking therefore we wrap it with `CompletableFuture.supplyAsync` and we use the JDBC blocking executor to run it. This is a pre-configured dispatcher provided by Pekko Projections. Its pool size can be configured with settings `pekko.projection.jdbc.blocking-jdbc-dispatcher.thread-pool-executor.fixed-pool-size` (see src/main/resources/persistence.conf).
...
[IMPORTANT]
====
Calls to the repository are blocking therefore we run them in a `Future` with the JDBC blocking execution context. This is a pre-configured dispatcher provided by Pekko Projections. Its pool size can be configured with settings `pekko.projection.jdbc.blocking-jdbc-dispatcher.thread-pool-executor.fixed-pool-size` (see src/main/resources/persistence.conf).
...
Create a xref:deployment:aws-install.adoc[Kubernetes cluster and install the Pekko Operator] if you haven't already.
...
Before following the steps below, create Kubernetes cluster and install the Pekko Operator. Used the instructions below for:
...
* {pekko}/typed/cluster-sharded-daemon-process.html[Pekko Sharded Daemon Process reference documentation {tab-icon}, window="tab"].
