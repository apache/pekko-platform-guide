...
[IMPORTANT]
====
Calls to the repository are blocking therefore we wrap it with `CompletableFuture.supplyAsync` and we use the blocking executor to run it. Make sure to follow that pattern whenever you interact with a DB repository or any blocking service. 
====
