...
.  [.group-java]#Use the Spring `ApplicationContext` to retrieve the implementation for the repository# [.group-scala]#Add the implementation for PostgresSQL by using the `ScalikeJdbcSession`:#
+
[.tabset]
Java::
+
...
<2> Call the initialization of the Projection.
...
Make sure that `ddl-scripts/create_user_tables.sql` is also loaded as previously described.
