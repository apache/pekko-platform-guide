...
. Create the Postgres tables from the SQL script located inside the `ddl-scripts` at the root of the project:
+
----
...
+
It will create all tables needed for Akka Persistence as well as the offset store table for Akka Projection.
