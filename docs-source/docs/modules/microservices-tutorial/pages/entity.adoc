...
The {pekko-blog}/news/2020/01/07/pekko-event-sourcing-video[Event Sourcing with Akka 2.6 video {tab-icon}, window="tab"] is also a good starting point for learning Event Sourcing.
...
The second video of the https://info.lightbend.com/pekko-platform-workshop-part-2-on-demand-recording.html[Akka Workshop Series {tab-icon}, window="tab"] covers both the cart entity and Event sourcing. It provides some solid guidance to aid you in digesting this section, and the next section of this guide.
...
See all available effects in the {pekko}/typed/persistence.html#effects-and-side-effects[Akka reference documentation {tab-icon}, window="tab"].
...
To glue the command handler, event handler, and state together, we need some initialization code. Our code will distribute the Cart entities over nodes in the Akka Cluster with https://doc.pekko.io/docs/pekko/current/cluster-sharding.html[Cluster Sharding {tab-icon}, window="tab"], enable snapshots to reduce recovery time when the entity is started, and restart with backoff in the case of failure.
...
You can learn more about the [.group-scala]#`EventSourcedBehaviorTestKit`# [.group-java]#`TestKitJunitResource`# in the {pekko}/typed/persistence-testing.html#unit-testing[Akka reference documentation {tab-icon}, window="tab"]
...
* {pekko}/typed/persistence.html[Akka Event Sourcing reference documentation {tab-icon}, window="tab"].
* {pekko}/typed/cluster-sharding.html[Akka Cluster Sharding reference documentation {tab-icon}, window="tab"].
