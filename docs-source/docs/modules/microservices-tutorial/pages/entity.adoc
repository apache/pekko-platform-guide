...
* distribute the entities over the nodes in the Pekko Cluster
...
The {akka-blog}/news/2020/01/07/akka-event-sourcing-video[Event Sourcing with Pekko 2.6 video {tab-icon}, window="tab"] is also a good starting point for learning Event Sourcing.
...
The second video of the https://info.lightbend.com/akka-platform-workshop-part-2-on-demand-recording.html[Pekko Workshop Series {tab-icon}, window="tab"] covers both the cart entity and Event sourcing. It provides some solid guidance to aid you in digesting this section, and the next section of this guide.
...
See all available effects in the {pekko}/typed/persistence.html#effects-and-side-effects[Pekko reference documentation {tab-icon}, window="tab"].
...
To glue the command handler, event handler, and state together, we need some initialization code. Our code will distribute the Cart entities over nodes in the Pekko Cluster with https://doc.pekko.io/docs/pekko/current/cluster-sharding.html[Cluster Sharding {tab-icon}, window="tab"], enable snapshots to reduce recovery time when the entity is started, and restart with backoff in the case of failure.
...
----
+
<1> The entities are distributed over the nodes in the Pekko Cluster with Cluster Sharding.
...
The state, commands and events of the entity must be serializable because they are written to the datastore or sent between nodes within the Pekko cluster. The template project includes built-in CBOR serialization. This section describes how serialization is implemented. You do not need to do anything specific to take advantage of CBOR, but this section explains how it is included.
...
You can learn more about the [.group-scala]#`EventSourcedBehaviorTestKit`# [.group-java]#`TestKitJunitResource`# in the {pekko}/typed/persistence-testing.html#unit-testing[Pekko reference documentation {tab-icon}, window="tab"]
...
* {pekko}/typed/persistence.html[Pekko Event Sourcing reference documentation {tab-icon}, window="tab"].
* {pekko}/typed/cluster-sharding.html[Pekko Cluster Sharding reference documentation {tab-icon}, window="tab"].
