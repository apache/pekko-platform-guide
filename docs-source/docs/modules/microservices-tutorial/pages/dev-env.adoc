...
This tutorial uses a Postgres database and Kafka. You may run local installations during development, but we recommend running both services using https://docs.docker.com[Docker {tab-icon}, window="tab"]. The template project's root directory includes a https://docs.docker.com/compose/[Docker Compose {tab-icon}, window="tab"] file that you can use to run Postgres and Kafka. The provided Docker Compose file also runs Zookeeper which is required to run a Kafka broker.
